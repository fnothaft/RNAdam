  ...
  def computeVertexMapping(graph: Graph[ColoredKmerVertex, Unit]): Map[Long, Map[String, Long]] = {
    ...
    VertexMapping.time { 
      graph.vertices                                                            // RDD[ kmerHash, ColoredKmerVertex ]                         
           .flatMap(v => if (v._2.stronglyConnected.isEmpty) {
                            for { t <- v._2.terminals } yield (v._1, (t._1, 1L)) }
                         else { 
                            for {t <- v._2.stronglyConnected} yield (v._1, (t._1._1, 1L))}) // RDD[ kmerHash, (color, 1) ]
